...
namespace LXMS
{
    public partial class ASSET_EVALUATE : Form
    {
        dalAssetEvaluate _dal = new dalAssetEvaluate();
        public ASSET_EVALUATE()
        {
            InitializeComponent();
        }

        private void ASSET_EVALUATE_Load(object sender, EventArgs e)
        {
        ...
        private void LoadData()
        {
            try
            {
                ...
                BindingCollection<modAssetEvaluate> list = _dal.GetIList(rbStatus0.Checked ? "0" : "1", string.Empty, string.Empty, string.Empty, rbStatus0.Checked ? string.Empty : dtpFrom.Text, rbStatus0.Checked ? string.Empty : dtpTo.Text, out Util.emsg);
