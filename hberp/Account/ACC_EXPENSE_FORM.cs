...
namespace LXMS
{
    public partial class ACC_EXPENSE_FORM : Form
    {
        dalAccExpenseForm _dal = new dalAccExpenseForm();
        public ACC_EXPENSE_FORM()
        {
            InitializeComponent();
        }

        private void ACC_EXPENSE_FORM_Load(object sender, EventArgs e)
        {
        ...
        private void LoadData()
        {
            try
            {
                ...
                BindingCollection<modAccExpenseForm> list = _dal.GetIList(rbStatus0.Checked ? "0" : "1", string.Empty, string.Empty, string.Empty, rbStatus0.Checked ? string.Empty : dtpFrom.Text, rbStatus0.Checked ? string.Empty : dtpTo.Text, out Util.emsg);
        ...
        private void toolNew_Click(object sender, EventArgs e)
        {
            ...
            {
                this.Cursor = Cursors.WaitCursor;
                EditAccExpenseForm frm = new EditAccExpenseForm();
        ...
        private void toolEdit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                modAccExpenseForm mod = (modAccExpenseForm)DBGrid.CurrentRow.DataBoundItem;
                EditAccExpenseForm frm = new EditAccExpenseForm();
                frm.EditItem(mod.Id);
                if (frm.ShowDialog() == DialogResult.OK)
                {
        ...
        private void toolDel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                modAccExpenseForm mod = _dal.GetItem(Convert.ToInt32(DBGrid.CurrentRow.Cells[0].Value), out Util.emsg);
                if (mod.Status == 1)
                {
        ...
        private void toolAudit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        modAccExpenseForm mod = _dal.GetItem(Convert.ToInt32(DBGrid.SelectedRows[i].Cells[0].Value), out Util.emsg);
                        if (_dal.Audit(mod.Id, Util.UserId, out Util.emsg))
                        {
