...
namespace LXMS
{
    public partial class EditAccOtherPayableForm : Form
    {
        string _action;
        dalAccOtherPayableForm _dal = new dalAccOtherPayableForm();
        INIClass ini = new INIClass(Util.INI_FILE);
        public EditAccOtherPayableForm()
        {
            InitializeComponent();
            FillControl.FillCheckType(cboCheckType, false, true);
            FillControl.FillBankList(cboBankName, false, true);
            FillControl.FillOtherPayableType(cboFormType, false);
        }

        private void EditAccOtherPayableForm_Load(object sender, EventArgs e)
        {
            clsTranslate.InitLanguage(this);            
            txtId.ReadOnly = true;
            if (clsLxms.GetParameterValue("NEED_OTHER_PAYABLE_NO").CompareTo("T") == 0)
        ...
        private void toolSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (clsLxms.GetParameterValue("NEED_OTHER_PAYABLE_NO").CompareTo("T")==0 && string.IsNullOrEmpty(txtNo.Text))
                {
                ...
                modAccOtherPayableForm mod = new modAccOtherPayableForm();
