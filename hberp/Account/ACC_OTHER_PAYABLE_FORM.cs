...
namespace LXMS
{
    public partial class ACC_OTHER_PAYABLE_FORM : Form
    {
        dalAccOtherPayableForm _dal = new dalAccOtherPayableForm();
        public ACC_OTHER_PAYABLE_FORM()
        {
            InitializeComponent();
        }

        private void ACC_OTHER_PAYABLE_FORM_Load(object sender, EventArgs e)
        {
        ...
        private void LoadData()
        {
            try
            {
                ...
                BindingCollection<modAccOtherPayableForm> list = _dal.GetIList(rbStatus0.Checked ? "0" : "1", string.Empty, string.Empty, rbStatus0.Checked ? string.Empty : dtpFrom.Text, rbStatus0.Checked ? string.Empty : dtpTo.Text, out Util.emsg);
        ...
        private void toolNew_Click(object sender, EventArgs e)
        {
            ...
            {
                this.Cursor = Cursors.WaitCursor;
                EditAccOtherPayableForm frm = new EditAccOtherPayableForm();
        ...
        private void toolEdit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                modAccOtherPayableForm mod = (modAccOtherPayableForm)DBGrid.CurrentRow.DataBoundItem;
                EditAccOtherPayableForm frm = new EditAccOtherPayableForm();
                frm.EditItem(mod.Id);
                if (frm.ShowDialog() == DialogResult.OK)
                {
        ...
        private void toolAudit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        modAccOtherPayableForm mod = _dal.GetItem(Convert.ToInt32(DBGrid.SelectedRows[i].Cells[0].Value), out Util.emsg);
                        if (_dal.Audit(mod.Id, Util.UserId, out Util.emsg))
                        {
