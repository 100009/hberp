...
namespace LXMS
{
    public partial class ACC_OTHER_RECEIVABLE_FORM : Form
    {
        dalAccOtherReceivableForm _dal = new dalAccOtherReceivableForm();
        public ACC_OTHER_RECEIVABLE_FORM()
        {
            InitializeComponent();
        }

        private void ACC_OTHER_RECEIVABLE_FORM_Load(object sender, EventArgs e)
        {
        ...
        private void toolNew_Click(object sender, EventArgs e)
        {
            ...
            {
                this.Cursor = Cursors.WaitCursor;
                EditAccOtherReceivableForm frm = new EditAccOtherReceivableForm();
        ...
        private void toolEdit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                modAccOtherReceivableForm mod = (modAccOtherReceivableForm)DBGrid.CurrentRow.DataBoundItem;
                EditAccOtherReceivableForm frm = new EditAccOtherReceivableForm();
                frm.EditItem(mod.Id);
                if (frm.ShowDialog() == DialogResult.OK)
                {
        ...
        private void toolDel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                modAccOtherReceivableForm mod = _dal.GetItem(Convert.ToInt32(DBGrid.CurrentRow.Cells[0].Value), out Util.emsg);
                if (mod.Status == 1)
                {
        ...
        private void toolAudit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        modAccOtherReceivableForm mod = _dal.GetItem(Convert.ToInt32(DBGrid.SelectedRows[i].Cells[0].Value), out Util.emsg);
                        if (_dal.Audit(mod.Id, Util.UserId, out Util.emsg))
                        {
