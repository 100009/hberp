...
namespace LXMS
{
    ...
    public class RWReg
    {
        ...
        /// <returns></returns>
        public string GetRegValue(string keypath, string keyname, string def)
        {
            try
            {
        ...
        public bool SetRegValue(string keypath, string keyname, string keyval)
        {
            try
            {
        ...
        /// <param name="keyname"></param>
        /// <param name="keyval"></param>
        public bool SetRegValue(string keypath, string keyname, string keyval, string keytype)
        {
            try
            {
                RegistryKey key = rootkey.OpenSubKey(keypath, true);
                RegistryValueKind ktype;
                switch (keytype.ToUpper())
                {
                    case "BINARY":
                        ktype = RegistryValueKind.Binary;
                        break;
                    case "DWORD":
                        ktype = RegistryValueKind.DWord;
                        break;
                    case "EXPANDSTRING":
                        ktype = RegistryValueKind.ExpandString;
                        break;
                    case "MULTISTRING":
                        ktype = RegistryValueKind.MultiString;
                        break;
                    case "QWORD":
                        ktype = RegistryValueKind.QWord;
                        break;
                    case "NONE":
                        ktype = RegistryValueKind.None;
                        break;
                    case "UNKNOWN":
                        ktype = RegistryValueKind.Unknown;
                        break;          
                    default:
                        ktype = RegistryValueKind.String;
                        break;
                }
                key.SetValue(keyname, (object)keyval, ktype);
                return true;
            }
            catch
            {
        ...
        /// <param name="keyname"></param>
        /// <returns></returns>
        public bool DelRegKeyValue(string keypath, string keyname)
        {
            try
            {
                RegistryKey key = rootkey.OpenSubKey(keypath, true);
