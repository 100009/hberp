...
namespace Encrypt
{
    public class XmlHelper
    {
        ...
        /// </summary>
        /// <returns></returns>
        public DataTable XmlToDataTableByFile(string XMLPath)
        {
            XmlDocument doc = new XmlDocument();
            doc.Load(XMLPath);
            DataTable dt = new DataTable("Person");
            //以第一个元素Person的子元素建立表结构       
            XmlNode songNode = doc.SelectSingleNode("/User/Person[1]");
            string colName;
            if (songNode != null)
            {
            ...
            ds.Tables.Add(dt);
            //Xml所有Person元素的子元素读到表Person中,当然用dt也可以读。  
            ds.ReadXml(XMLPath);
            return dt;
        }
        ...
        /// <param name="Element">元素</param>
        /// <param name="Content">内容</param>
        public void InsertNode(string MainNode, string ChildNode, List<Dictionary<string, string>> dic)
        {
            //插入一节点和此节点的一子节点。 
            XmlNode objRootNode = objXmlDoc.SelectSingleNode(MainNode);
            XmlElement objChildNode = objXmlDoc.CreateElement(ChildNode);
            ...
            {
                XmlElement objElement = objXmlDoc.CreateElement(item["element"].ToString());
                objElement.InnerText = item["Content"].ToString();
                objChildNode.AppendChild(objElement);
            }
        }
